...
class FrotaController extends Controller
{
    ...
    public function dadosGerais(){
      ...
      $baixadas = Viatura::where('situacao_viatura_id',4)->where('opm_id','=',$usr->efetivo->opm->id)->count();      
    ...
    public function lista()
      {
        $this->dadosGerais();
        $usr = Auth::user();
        $viaturas = Viatura::where('opm_id','=',$usr->efetivo->opm->id)->orderBy('prefixo')->paginate($this->totalPage);
    ...
    {  
      ...
      try{
        if($request->id != null)
          ...
          DB::insert('insert into viatura_km (viatura_id, data, hora,km, user_id) values (?,?,?,?,?)',
     ...
     // dd($request->all());
      ...
      try{
        if($request->id != null)
          ...
          DB::insert('insert into viatura_revisao (viatura_id, data, local,km,servicos, user_id) values (?,?,?,?,?,?)',
    ...
    {  
      ...
      try{
        if($request->id != null)
          ...
          DB::insert('insert into historico_viatura (viatura_id, data, observacao,tipo_historico_viatura_id,user_id) values (?,?,?,?,?)',
